...
5. **`/to-spec`** transforme l'entretien en spec, écrite dans `docs/specs/<feature-slug>.md` quel que soit le tracker, et suivie sur un vrai tracker par une **epic** mince — titrée `spec: <feature-slug>`, labellisée `ready-for-agent`, qui ne fait que pointer vers le fichier. Il n'interviewe pas : il lit `decisions.md`, relève dans `docs/prd.md` les **exigences non fonctionnelles qui contraignent cette fonctionnalité** avec leur chiffre, reprend la carte d'un chantier `/wayfinder` s'il y en a une, puis esquisse les **seams** de test et te les fait valider. Une seule spec, celle de cette fonctionnalité.
...
7. **`/analyze`** confronte les artefacts **les uns aux autres**, avant qu'une ligne de code existe : une exigence chiffrée du PRD que la spec a perdue, une décision de spec qu'aucun ticket ne porte, un slug divergent entre spec, epic et branche, un ticket sans critère d'acceptation, une arête de blocage écrite en prose mais pas câblée. Trois axes en parallèle — Descente, Cohérence, Constructibilité. Il ne modifie rien : corriger ici coûte un `/to-tickets` relancé, le découvrir plus tard coûte une fonctionnalité refaite.

8. **`/implement`**, un ticket à la fois, **en faisant un `/clear` du contexte entre chacun**. Chaque ticket est autoportant, donc le contexte du précédent est jetable. Il pilote **`/tdd`** en interne (une tranche red-green à la fois, aux seams convenus), valide critère par critère avec une preuve nommable, puis relit avec **`/code-review`** — une revue du diff sur deux axes, Standards et Spec. La revue rapporte, elle ne corrige pas : le **refactor**, troisième temps du red-green-refactor que `/tdd` laisse de côté, a lieu ici, sur les constats de l'axe Standards, avant de commiter.
...
- **`/improve-codebase-architecture`** balaie le code à la recherche de modules **shallow**, rend les candidats dans un rapport HTML visuel avec un avant/après par candidat, puis grille celui que tu choisis. Il pondère les zones qui ont récemment bougé — approfondir se rentabilise sur les changements à venir, pas sur du code figé. Le prendre quand l'architecture freine, avant qu'une fonctionnalité ne s'y casse les dents ; pas pour une fonctionnalité déjà cadrée, où `/grill-with-docs` est la bonne porte.
- **`/wizard`** génère un script bash qui guide un **humain** à travers ce que lui seul peut faire : traverser un dashboard tiers, poser des identifiants ou des secrets de CI, mener une bascule ponctuelle. Ne pas l'invoquer pour ce que l'agent sait faire lui-même — c'est précisément la frontière qu'il matérialise.
- **`/to-questionnaire`** transforme une décision que tu ne peux pas trancher seul en un document Markdown destiné à celui qui détient le contexte manquant. Il ne t'interviewe que sur l'**envoi** — à qui, pour obtenir quoi — puis vise les questions sur l'écart.
- **`/retro`** relit une session déjà jouée et propose quoi changer dans l'**environnement** — pointeur de contexte manquant, vérification automatique absente, règle de revue à ajouter, directive inerte à supprimer. Il ne corrige pas la session : il rend la suivante moins coûteuse. Le prendre après une session qui a traîné ou dérapé, pas pendant.
