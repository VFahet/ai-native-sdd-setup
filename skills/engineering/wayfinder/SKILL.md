...
Le slug est en kebab-case et nomme la **fonctionnalité**, pas le chantier : il désigne le `docs/specs/<feature-slug>.md` où la spec sera écrite, et le `.scratch/<feature-slug>/` où `/to-tickets` écrira les tickets d'implémentation sur le tracker markdown local. Il peut reprendre le nom du chantier — `map.md` et `decisions/` cohabitent alors avec `issues/` — mais une carte qui appelle plusieurs specs a besoin d'un slug par spec.
