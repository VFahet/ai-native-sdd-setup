...
Lui passer d'emblée les deux choses qu'il demanderait sinon : le **point fixe**, c'est-à-dire le `HEAD` d'avant le travail du ticket, en signalant que ce travail n'est pas encore commité ; et la **référence du ticket** comme source de spec, sous la forme résolue à l'étape 1 — le chemin du fichier sur le tracker markdown local, jamais un numéro nu, que la revue ne sait pas résoudre à partir d'un glob. Sur un vrai tracker le numéro *est* la forme canonique : le passer tel quel, avec le nom du tracker, la revue sachant le résoudre via `docs/agents/issue-tracker.md`.
...
- **Source de spec** : la **spec** de la fonctionnalité — le fichier `docs/specs/<feature-slug>.md`, quel que soit le tracker. Sur un vrai tracker, l'epic dont les tickets descendent ne fait que pointer vers ce fichier : c'est le fichier qu'il faut lire. Surtout pas un ticket : c'est le couple trunk + spec qui fait tout l'intérêt de cette passe. **S'il n'y a pas de spec** — `/to-spec` a été sauté —, le dire à la revue au lieu de la laisser en chercher une : elle tourne alors sur son seul axe Standards, qui suffit à voir les incohérences entre tickets, et le rapport porte la mention que rien n'a confronté le diff à une intention.
...
**Le statut de la spec.** Avant d'ouvrir la PR, passer l'en-tête de `docs/specs/<feature-slug>.md` à `**Statut :** Implemented`, dans le même commit que les corrections de revue. C'est ce qui empêche le statut de rester bloqué à `Draft` : un champ qui n'avance jamais apprend au lecteur à l'ignorer, et `/code-review` s'en sert pour savoir si l'intention qu'il lit est encore celle du code. `Approved` reste un geste humain, posé avant l'implémentation ; `Superseded by …` aussi.
