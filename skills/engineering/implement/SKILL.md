...
Lire le ticket en entier — corps, critères d'acceptation, bloqueurs. Si l'utilisateur a passé un chemin ou un numéro, le récupérer via le workflow décrit dans `docs/agents/issue-tracker.md`. Sur un tracker markdown local, un numéro nu ne suffit pas à désigner un fichier : la chaîne prescrit un `/clear` entre chaque ticket, donc le `<feature-slug>` n'est plus en contexte. Parcourir `.scratch/*/issues/` à la recherche du fichier portant ce numéro, en écartant les tickets de décision d'un chantier `/wayfinder` : même chemin, même numérotation à partir de `01`, mais une ligne `Type:` (`research`/`prototype`/`grilling`/`task`) les distingue. Il n'en reste qu'un : l'annoncer et continuer. Plusieurs : demander lequel. Aucun : le dire et demander le chemin complet — implémenter le ticket de décision qui portait ce numéro livrerait du code là où le chantier cherchait encore une décision.
